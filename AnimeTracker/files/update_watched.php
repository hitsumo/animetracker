...
/**
 * Anime Tracker - Quick Watched-Episode Update Endpoint (0.5.5)
 ...
 *   }
 *
 * Note: watched_episodes is personal watch progress. In Faz 2
 * (multi-user) this endpoint must become user-scoped (write to the
 * per-user table, not the shared animes row). Logged in the Faz 2
 * "tasinacaklar" list alongside chronology_default_view.
 */
...
$stmt = $pdo->prepare(
    "SELECT watched_episodes, total_episodes, aired_episodes
       FROM animes
      WHERE id = ?"
);
...
$upd = $pdo->prepare(
    "UPDATE animes SET watched_episodes = ? WHERE id = ?"
);
$upd->execute([$new, $animeId]);
