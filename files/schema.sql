...
--   install_id         - random 32-hex token identifying this install to
--                        the project's install counter (1.1.43, written
--                        by install_ping_helpers.php on the first ping;
--                        never regenerated by the app - delete the row to
--                        start over as a "new" install)
--   last_install_ping  - UTC 'Y-m-d' of the last install-counter attempt
--                        (1.1.43); at most one attempt per day
--   install_ping_done  - UTC datetime of the one successful ping (1.1.43);
--                        once present, no further ping is ever sent
...
-- stored here as 'pending' (NOT inserted into animes), so the shared
-- catalog stays clean; a moderator promotes them into animes on approval.
-- Kept in sync with migration/{version}/upgrade.sql so a FRESH install
-- creates the table directly - a fresh install sets settings.version to
-- the current version and the runner skips older migrations, so this
-- table must live in the base schema, not only in the migration.
