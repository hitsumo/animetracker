...
-- --------------------------------------------------------
-- Table: catalog_requests (1.0.6 - Faz 2, online import suggestions)
--
-- When an online member runs "Import List", anime in their list that are
-- NOT yet in the shared catalog are written here as "pending" suggestions
-- (NOT into animes). This keeps the shared catalog (animes) clean; once a
-- moderator approves a row it is promoted into animes as a real row. Because
-- these rows never enter animes, the main list / statistics / FKs are
-- unaffected and privacy comes from the design.
--
--   - suggestion_status: 'pending' awaiting review; 'approved' promoted into
--     animes (the row is kept for audit); 'rejected' declined.
--   - suggested_by / reviewed_by: users.id (NULL if the user is deleted).
--   - The columns mirror the catalog columns in the export JSON so that on
--     approval a fully-populated animes row can be produced.
--   - On self-host (single-user) this table stays empty - import there is a
--     full backup restore and writes the catalog directly, no queue.
--
-- idx_creq_status backs the moderation queue; idx_creq_mal backs catalog
-- matching by MAL id; idx_creq_by backs "this user's suggestions".
-- --------------------------------------------------------
